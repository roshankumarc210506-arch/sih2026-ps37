...
%locked interface contract. Two are real (Perception via M1's Day-1 data,
%Control via M4's real Model Reference block); three are confirmed dummy
%stubs for Day 1 (Prediction, GlobalPlanner, DecisionLogic), matching
%today's team decisions.
%
...
% script.
    ...
    % Perception/Prediction/GlobalPlanner/DecisionLogic run at this same
    ...
    %% ---- Perception: From Workspace (M1's real data) ----
    p = [modelName '/Perception'];
    add_block('simulink/Ports & Subsystems/Subsystem', p, 'Position', [30 30 160 130]);
    delete_block([p '/In1']);
    delete_block([p '/Out1']);
    add_block('simulink/Sources/From Workspace', [p '/PerceptionFromMat'], ...
        'VariableName', 'perceptionData', 'Position', [30 20 190 60]);
    add_block('simulink/Sources/From Workspace', [p '/EgoFromMat'], ...
        'VariableName', 'egoData', 'Position', [30 90 190 130]);
    ...
    add_line(p, 'PerceptionFromMat/1', 'PerceptionOut/1', 'autorouting', 'on');
    add_line(p, 'EgoFromMat/1', 'EgoOut/1', 'autorouting', 'on');
    % NOTE(M6, Day 1): From Workspace output must be typed to SihPerceptionBus
    % / SihEgoBus via Signal Attributes -- do this by hand in the Simulink
    % editor (Block Parameters > Output > Bus object) since it's a GUI-only
    % setting on some MATLAB versions. Verify before treating Perception as
    % "real" rather than "wired but untyped."
    ...
    % Dummy constant matches SihPredictionBus via createMATLABStruct.

    %% ---- GlobalPlanner: CONFIRMED dummy stub (M3's real pipeline is not ----
    %% Simulink-safe yet -- variable-length structs, string class field.
    %% M3 explicitly asked to keep this as a stub through Day 1; M3 is
    %% converting internals to fixed-size/enum-typed today for Day 2-3. ----
    ...
    %% ---- DecisionLogic: CONFIRMED dummy stub (M5's Stateflow file path ----
    %% not yet confirmed in repo) ----
    ...
    % (fail-safe default) -- M4 was explicit this must not be changed to
    % CRUISE for convenience.

    %% ---- Control: REAL, via M4's Model Reference block ----
    % Per M4's explicit Day-1 instructions: use Model Reference (not a
    % nested Subsystem) since M4_Control.slx is binary and git can't
    % merge it -- this keeps M4's file and this top-level model as
    % separate files nobody collides on.
    ...
    % All bus ports nonvirtual -- required for Model Reference.
    ...
    add_line(modelName, 'Perception/2', 'Control/2', 'autorouting', 'on'); % ego pose -> ego_state
    ...
    % assignVelocityProfile's externalSpeedCap_mps argument. This is a
    % genuine feedback loop, not wired on this first pass -- GlobalPlanner
    % is still a stub today so there's nothing real to feed yet. Add a
    % Memory/unit-delay block on this signal when GlobalPlanner goes real,
    % so mode changes latch into the NEXT replan cycle rather than creating
    % a same-step algebraic loop between two subsystems running at
    % different rates.

    save_system(modelName, fullfile(pwd, 'model', [modelName '.slx']));
    fprintf('\nBuilt %s.slx.\n', modelName);
    fprintf('REAL today: Perception (M1 data, needs manual bus-type check), Control (M4 Model Reference).\n');
    fprintf('STUB today (confirmed with owners): Prediction, GlobalPlanner, DecisionLogic.\n');
